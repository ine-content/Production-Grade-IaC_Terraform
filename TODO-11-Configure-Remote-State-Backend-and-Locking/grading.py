...
TODO 01, and stops at the first incomplete one - even though TODO 01-10
...
lab (see _scoped_workdir()), exactly like TODO 02-10 - the student's real
...
# TODO 11 is the one exception to the truncation scheme above: its own
# STUDENT WORK AREA lives INSIDE the shared terraform {} block at the very
# top of main.tf (a backend can only be declared inside that same block -
# see main.tf itself), not appended after the last TODO's own content the
# way every earlier TODO's work area is. Truncating main.tf partway
# through an still-open terraform {} block would leave it with no closing
# brace - invalid HCL no matter what came before the cut - so there is no
# safe marker to truncate before here, and none is needed: a backend
# block left out entirely is completely valid HCL (Terraform just falls
# back to local state), so main.tf validates fine before TODO 11 is
# started, while it's being written, and once it's correct. See
# ensure_valid().
...
def ensure_valid():
    ...
    Every earlier TODO's grading.py validated a copy truncated right
    after the CURRENT (latest, still-blank) TODO's own END marker, since
    an unwritten later TODO's placeholder wouldn't be valid HCL on its
    own, and shouldn't be blamed on an earlier one. TODO 11 doesn't need
    that: TODO 01-10 are all fully solved in this folder already, nothing
    trails TODO 10's own output, and TODO 11's own STUDENT WORK AREA
    lives inside the existing terraform {} block as commented-out
    instructions - leaving it out entirely is completely valid HCL
    (Terraform just falls back to local state), so main.tf validates
    fine whether TODO 11 is untouched, correct, or mid-edit with a real
    mistake (which SHOULD show up here, the same as breaking any other
    TODO's own active work would). So this validates the full, untruncated
...
def check_todo_11():
    ...
    check in this course. Runs against the full, untruncated file (see
    ensure_valid() for why TODO 11 needs no truncation marker)."""
    if not HAVE_BOTO3:
        return False
...
def print_localstack_unreachable():
    ...
    console.print(c(
        f"Couldn't reach {LOCALSTACK_HEALTH_URL} - TODO 06 through TODO 11 all need "
...
def print_boto3_missing():
    ...
    console.print(c(
        "TODO 10 and TODO 11 both use boto3 to call the AWS API directly, "
        "independently of Terraform. Install it, then run python grading.py "
        "again.",
        YELLOW,
    ))
    console.print()
