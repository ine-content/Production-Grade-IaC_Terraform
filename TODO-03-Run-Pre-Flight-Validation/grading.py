...
(environments/staging.json disabling a zone role, "gust", that was never
defined in the service intent), running terraform apply, and confirming
it fails - proof your precondition is actually being evaluated, not just
present as an inert `condition = true` that would pass no matter what.
The file is always restored afterward, and a second, clean apply must
then succeed. This check only runs at all once TODO 01 and TODO 02 are
already confirmed correct, so a failure here always means TODO 03
specifically, never noise from an earlier TODO.

A single `terraform apply` evaluates every output in main.tf at once, so
a broken TODO 03 (e.g. a precondition that fails on valid data) would
otherwise fail the whole apply and make TODO 02 look broken too, even
though TODO 02's own locals are fine. `-target` can't fix this - it only
addresses resources, data sources, and module calls, never locals or
outputs. Instead, TODO 02's check runs against a disposable scratch copy
of this lab with everything from TODO 03 onward physically cut out of
main.tf first (see _scoped_workdir()) - the student's real
terraform.tfstate in this directory is never touched by grading.
...
# Cutting main.tf here, when grading TODO 02, drops TODO 03's output and
# precondition entirely - see _scoped_workdir() below for why.
TODO_03_MARKER = "# STUDENT WORK AREA - TODO 03"
...
def _scoped_workdir(truncate_before=None):
    ...
    cutting main.tf short right before a marker string.

    -target can't be used for this course's isolation problem - Terraform
    only allows targeting resources, data sources, and module calls, not
    locals or outputs (confirmed against Terraform's own CLI docs), and
    TODO 02 and TODO 03 are nothing but locals and outputs. So instead:
    copy the lab into a scratch directory and physically remove
    everything from TODO 03 onward from that copy's main.tf before
    running terraform in it. A precondition that's broken, or missing
    entirely, in the part of the file that got cut away can't affect a
    check for an earlier TODO, because that code was never even there to
    evaluate. The real ROOT directory (and the student's real
    terraform.tfstate in it) is never touched by this - every scoped
    check runs in its own disposable copy."""
...
def apply_and_read_output(name, truncate_before=None):
    """Run init + apply in a scoped scratch copy of this lab (see
    _scoped_workdir), then read one output back. Returns the parsed
    output value, or None if init/apply/output failed for any reason."""
...
def ensure_valid():
    """`terraform validate`, run against the real ROOT directory, checks
    that main.tf (as it stands right now - every TODO's code, whatever
    state it's in) is structurally sound HCL, WITHOUT evaluating whether
    any precondition's condition is actually true. That makes it the
    right tool to catch a genuine syntax mistake (a missing brace, a bad
    block header) before ever attributing a failure to a specific TODO: a
    syntax error anywhere in the file stops every TODO from running at
    all, so it isn't any one TODO's fault. Memoized - the file doesn't
    change between calls within one grading.py run except inside
    check_todo_3's own temporary edits, which touch a .json data file,
    never main.tf."""
    ...
    rc, init_out, init_err = run_terraform(ROOT, "init", "-input=false", "-no-color")
    if rc != 0:
        _validate_result = (False, (init_err or init_out or "terraform init failed").strip())
        return _validate_result

    rc, stdout, stderr = run_terraform(ROOT, "validate", "-no-color", "-json")
    if rc == 0:
        _validate_result = (True, "")
        return _validate_result
...
def apply_in_scoped_copy():
    ...
    without ever touching the student's real terraform.tfstate."""
    workdir = _scoped_workdir()
    try:
...
def check_todo_2():
    """Runs in a scratch copy of the lab with everything from TODO 03
    onward cut out of main.tf first - see _scoped_workdir(). That means
    this check's result depends only on TODO 01 and TODO 02's own code,
    never on whatever state TODO 03 happens to be in."""
...
def check_todo_3():
    ...
    block even if something raises. Each apply_in_scoped_copy() call
    copies whatever is on disk *at that moment* into its own scratch
    directory and applies there, so the student's real terraform.tfstate
    in ROOT is never touched by this back-and-forth."""
    ...
    try:
        ...
        broken_rc, _, _ = apply_in_scoped_copy()
    ...
    clean_rc, _, _ = apply_in_scoped_copy()
    return clean_rc == 0
