...
TODO 01, and stops at the first incomplete one - even though TODO 01-08
...
solved) added one small compliance tag (via aws_ec2_tag, a standalone
resource) and proved the exact same configuration applied twice produces
zero further changes. TODO 09 introduces a genuinely different kind of
block - a `data` source, which never creates or manages anything; it only
reads, fresh, from the provider, on every single plan and apply - and
uses one to independently confirm what aws_instance.branch_gateway
already claims about itself is what's actually running in AWS. Every
check below still runs against a disposable scratch copy of this lab
(see _scoped_workdir()), exactly like TODO 02-08 - the student's real
...
apply_read_and_destroy() and apply_and_check_idempotent()).
...
TODO_09_MARKER = "# STUDENT WORK AREA - TODO 09"

# The data source and check block after TODO 09's own work area reference
...
# state before TODO 09 is solved - not a syntax error - so ensure_valid()
...
# same reason).
CURRENT_TODO_END_MARKER = "# END STUDENT WORK AREA - TODO 09"
...
def check_todo_9():
    ...
    apply_read_and_destroy()). Runs against the full, untruncated file -
    TODO 09 is the last TODO in this folder - the same real-apply
    approach as check_todo_6, check_todo_7, and check_todo_8."""
...
def print_localstack_unreachable():
    ...
    console.print(c(
        f"Couldn't reach {LOCALSTACK_HEALTH_URL} - TODO 06 through TODO 09 all need "
