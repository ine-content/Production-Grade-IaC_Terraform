terraform {
  ...
  # ---------------------------------------------------------------------------
  # TODO 11 - already solved in this folder.
  # ---------------------------------------------------------------------------
  backend "s3" {
...
# every plan/apply down while they time out. s3_use_path_style matters
# starting this TODO specifically: LocalStack doesn't answer to
# virtual-hosted-style bucket subdomains (the AWS SDK default) out of the
# box, only bucket-in-the-path addressing - the exact same reason TODO
# 11's own backend block sets its own, separate use_path_style = true.
#
...
# ---------------------------------------------------------------------------
# STUDENT WORK AREA - TODO 12
# ---------------------------------------------------------------------------
# Terraform's own state only ever tracks the CURRENT desired shape of this
# infrastructure - it has no memory of what happened on any earlier apply,
# and every apply overwrites it. Meridian Retail's compliance team wants a
# durable, append-only record of every apply instead - kept somewhere state
# can't touch, since state gets overwritten and audit trails can't be.
#
# audit/apply_log.json already exists, and already has one timestamp in
# it - proof this pipeline has been applied before. Every time this
# pipeline gets applied again, one more timestamp gets appended to that
# same file (the grader does this for you - representing time actually
# passing between real applies), and your job is to make sure every
# timestamp that has EVER appeared in that file gets its own permanent,
# independent record in S3 - never overwritten, never merged into a single
# "latest" file.
#
# Define exactly these 2 things - top-level, not nested inside anything
# else. Every name below is exactly what the grader looks for - do not
# rename them.
#
#   local.audit_entries   - every timestamp currently in
#     audit/apply_log.json, as a set (not a list) of strings
#
#   aws_s3_object "apply_audit_log"   - one real S3 object PER timestamp
#     in local.audit_entries, for_each over that same set
#
# Here's the part worth understanding before you write this: aws_s3_object
# treats its own key argument as part of that object's fundamental
# identity - AWS itself has no notion of "renaming" an object, so changing
# key forces Terraform to delete the old object and create a brand new one
# under the new key. That means a single aws_s3_object whose key embeds
# something that changes on every apply (timestamp() evaluated fresh each
# time, for instance) would DELETE its own previous entry every time -
# the opposite of an audit trail. for_each avoids this entirely: each
# instance's own key (in Terraform's own for_each sense) stays fixed once
# it exists, so its key argument (in the AWS S3 sense) never changes
# either - existing instances are left alone, and only a genuinely NEW
# timestamp ever produces a genuinely NEW object.
#
# See TASK.md for the exact attributes this needs, and for how the grader
# proves old entries survive a second apply untouched.

# ---------------------------------------------------------------------------
# END STUDENT WORK AREA - TODO 12
# ---------------------------------------------------------------------------
...
# ===========================================================================
#
# Wraps aws_s3_object.apply_audit_log in an output, so the grader (and
# you) can read back every audit key that currently exists without
# touching Terraform's own state file, or AWS, directly:
#
#   terraform output -json apply_audit_log_keys
#
# The grader reads this exact same output to check TODO 12's work - never
# by reading your main.tf source as text.
output "apply_audit_log_keys" {
  value = sort([for k, o in aws_s3_object.apply_audit_log : o.key])
}
