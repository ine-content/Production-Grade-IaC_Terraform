...
TODO 01, and stops at the first incomplete one - even though TODO 01-11
...
actually running in AWS. TODO 10 (also already solved) uses boto3 to call
the AWS API directly - completely bypassing Terraform, exactly like
...
independently via boto3 again afterward (see check_todo_10()). TODO 11
goes further still: it configures a real `backend "s3"` block, with
DynamoDB locking, inside main.tf's own top-level `terraform {}` block -
the one block every TODO's own scoped copy always includes in full, no
matter where main.tf gets truncated for that TODO's check. Terraform can
never create the bucket and lock table its own backend depends on (that's
a chicken-and-egg problem no Terraform code can solve), so this grader
creates both directly via boto3, once, unconditionally, before ANY check
below runs - not just check_todo_11() (see ensure_backend_infra(), called
from main()). check_todo_11() itself never reads main.tf's own source as
text either - it reads back `.terraform/terraform.tfstate`, the small
pointer file `terraform init` itself writes once a real backend is
configured, to independently confirm Terraform resolved the exact bucket,
key, region, and lock table this course requires.

TODO 12 goes one step further again: it writes a genuinely append-only
audit trail into that same S3 bucket, via `aws_s3_object` `for_each` over
every timestamp in audit/apply_log.json. `aws_s3_object` treats `bucket`
and `key` as its own defining identity - AWS itself has no notion of
renaming an object, so changing `key` forces Terraform to delete the old
object and create a new one (confirmed directly against the AWS provider's
own source, not assumed - a single resource whose key embeds
`timestamp()` would delete its own previous entry every apply, the
opposite of an audit trail). `for_each` avoids this: each instance's own
key stays fixed once it exists, so nothing about it ever changes, and only
a genuinely new timestamp ever produces a genuinely new object. This
grader proves that directly: it applies once, appends a new timestamp to
a scratch copy of audit/apply_log.json, applies again against that same
state, and confirms via the plan's own resource_changes that every
earlier timestamp's object planned as "no-op" while only the new one
planned as "create" (see check_todo_12()) - then independently confirms,
via boto3, that every object this course expects genuinely exists in S3
with the right content.

Every check below still runs against a disposable scratch copy of this
lab (see _scoped_workdir()), exactly like TODO 02-11 - the student's real
...
apply_read_and_destroy(), apply_and_check_idempotent(), check_todo_10(),
check_todo_11(), and check_todo_12()).

Requires the `terraform` CLI (>= 1.6, for the S3 backend's `endpoints`
block syntax) on PATH, LocalStack running at http://localhost:4566, and
...
TODO_10_MARKER = "# TODO 10 - already solved in this folder."
TODO_12_MARKER = "# STUDENT WORK AREA - TODO 12"

# TODO 11 has no marker of its own in this list: its content lives INSIDE
# the shared terraform {} block at the very top of main.tf (a backend can
# only be declared inside that same block - see main.tf itself), not
# appended after the last TODO's own content the way every other TODO's
# work area is. There's nothing to truncate before for TODO 11 specifically
# - once it's solved (as it is in this folder), its content is just a few
# more lines inside a block every scoped copy already includes in full
# regardless of where main.tf gets cut.
#
# TODO 12's own local and resource reference audit/apply_log.json and
# local.sites/local.branch_gateway_devices - all of which already exist
# (TODO 02 and TODO 05 are solved), so referencing them is never the
# problem. But a block with a syntax mistake, or none written at all, is a
# completely normal, expected state before TODO 12 is solved - not a
# syntax error - so ensure_valid() validates only up through here. See
# ensure_valid() for why this matters (same fix as TODO 04's grading.py,
# now applied here again for the same reason).
CURRENT_TODO_END_MARKER = "# END STUDENT WORK AREA - TODO 12"
...
# LocalStack, region, and credentials for the boto3 clients TODO 10 and
# TODO 11's grading use - the exact same endpoint and dummy credentials
# the aws provider itself already points at (see main.tf's own provider
# block).
LOCALSTACK_ENDPOINT_URL = "http://localhost:4566"
AWS_REGION = "us-east-1"

# TODO 11's required backend configuration - fixed, literal values (a
# Terraform backend block can never use a variable, a local, or any kind
# of expression, so there's nothing to compute here). ensure_backend_infra()
# creates the bucket and table these name directly via boto3, since
# Terraform itself can never create the backend it depends on.
BACKEND_BUCKET = "meridian-retail-tfstate"
BACKEND_KEY = "branch-network/terraform.tfstate"
BACKEND_REGION = AWS_REGION
BACKEND_DYNAMODB_TABLE = "meridian-retail-tf-locks"

# TODO 12 writes its audit trail into the SAME bucket TODO 11's backend
# already bootstrapped (see ensure_backend_infra()) - no new bucket, no
# new bootstrap step, just a different key prefix so audit objects never
# collide with the backend's own state object.
AUDIT_LOG_FILE = ROOT / "audit" / "apply_log.json"
AUDIT_KEY_PREFIX = "audit-log/"
...
TODO_NAMES = {
    ...
    11: "Configure Remote State Backend & Locking",
    12: "Emit a Structured Audit Log",
}
...
SUCCESS_MESSAGES = {
    ...
    11: "main.tf's terraform {} block now configures a real S3 backend with DynamoDB locking - Terraform itself resolved the exact bucket, key, region, and lock table this course requires, and state is confirmed living in S3, not on local disk.",
    12: "aws_s3_object.apply_audit_log writes one independent, permanent record per apply into S3 - verified by applying twice and confirming every earlier record survived untouched while exactly one new record appeared.",
}
...
FAIL_MESSAGES = {
    ...
    11: "The pipeline cannot continue because a remote state backend with locking has not been correctly configured yet.",
    12: "The pipeline cannot continue because a structured, append-only audit log has not been correctly emitted yet.",
}
...
HINTS = {
    ...
    11: "Add a backend \"s3\" block INSIDE the existing terraform {} block at the very top of main.tf, right after required_providers - never as its own separate top-level block; Terraform only allows one terraform {} block per file. Every argument has to be a literal value, never an expression: bucket = \"meridian-retail-tfstate\", key = \"branch-network/terraform.tfstate\", region = \"us-east-1\", dynamodb_table = \"meridian-retail-tf-locks\". A backend block authenticates completely separately from the aws provider block below it, so it needs its own access_key/secret_key (\"test\"/\"test\", same dummy values) plus the same 3 skip_* flags, use_path_style = true, and an endpoints block pointing both s3 and dynamodb at http://localhost:4566.",
    12: "local.audit_entries should be toset(jsondecode(file(\"${path.module}/audit/apply_log.json\"))) - toset(), not just the raw list, since for_each requires a set or a map. Then resource \"aws_s3_object\" \"apply_audit_log\" needs for_each = local.audit_entries, bucket = \"meridian-retail-tfstate\", key = \"audit-log/${each.value}.json\", and content = jsonencode({...}) built from applied_at = each.value plus whatever else you want recorded (local.sites, local.branch_gateway_devices are both already available). Never build key from timestamp() directly on a single resource - see the comment above the STUDENT WORK AREA for why that would delete every previous entry instead of adding to them.",
}
...
resource "aws_s3_object" "apply_audit_log" {
  for_each     = local.audit_entries
  bucket       = "meridian-retail-tfstate"
  key          = "audit-log/${each.value}.json"
  content_type = "application/json"

  content = jsonencode({
    applied_at    = each.value
    site_count    = length(local.sites)
    gateway_count = length(local.branch_gateway_devices)
    sites         = keys(local.sites)
  })
}''',
}
...
EXPECTED = {
    ...
    11: "terraform init should resolve a backend of type \"s3\", with config.bucket == \"meridian-retail-tfstate\", config.key == \"branch-network/terraform.tfstate\", and config.dynamodb_table == \"meridian-retail-tf-locks\" - read from .terraform/terraform.tfstate, the pointer file terraform init itself writes, never from main.tf's own source. A real terraform apply should succeed against that backend, no local terraform.tfstate file should exist afterward in the working directory, and the state object should be independently confirmed to exist in the S3 bucket at that exact key via the AWS API directly.",
    12: "terraform output -json apply_audit_log_keys should return exactly one \"audit-log/<timestamp>.json\" key per timestamp currently in audit/apply_log.json, each independently confirmed via the AWS API to exist in S3 with the right site_count, gateway_count, and sites. After a new timestamp is appended to that file and applied again against the same state, every previously-existing audit object's own planned action should be \"no-op\" (never touched, never replaced) while exactly the new timestamp's object plans as \"create\" - and after that second apply, all objects (old and new) should still independently exist in S3.",
}
...
PROBLEMS = {
    ...
    11: "backend \"s3\" is missing from the top terraform {} block, is declared as its own separate block instead of inside the existing one, is missing one or more required arguments, references the wrong bucket/key/dynamodb_table, or state still ends up in a local terraform.tfstate file instead of genuinely landing in S3.",
    12: "local.audit_entries is missing, isn't a set (toset() was skipped), or aws_s3_object.apply_audit_log is missing, isn't for_each'd over local.audit_entries, uses the wrong bucket or key, or builds key from something other than each.value - meaning an existing audit entry can get destroyed and recreated (or all entries collapse onto a single key) instead of every past entry surviving untouched.",
}
...
def run_terraform(cwd, *args):
    ...
    try:
        ...
        return 127, "", "terraform CLI not found on PATH. Install Terraform >= 1.6 to run this grader."
...
def ensure_valid():
    ...
    This validates a scoped copy truncated right after TODO 12's own END
    marker - NOT the raw ROOT directory - because a not-yet-written TODO
    12 is a completely normal, expected state, not a syntax error, and
    should never be confused for one. Any syntax mistake actually made
    while writing TODO 12 is still inside the truncated copy (that code
    comes before the END marker), so this still catches real syntax
    errors just fine. TODO 11 needs no special handling here - its own
    content lives inside the shared terraform {} block at the very top of
    main.tf, which this truncated copy always includes in full regardless
    of where the cut happens further down, and a backend block left out
    entirely (or written correctly, since TODO 11 is solved in this
    folder) is valid HCL either way. See TODO 04's grading.py, which hit
    this exact truncation bug first.
...
def check_todo_10():
    ...
    workdir = _scoped_workdir(truncate_before=TODO_12_MARKER)
    try:
...
def _localstack_s3_client():
    """A boto3 S3 client pointed directly at LocalStack. path addressing
    (bucket-in-path, not bucket-as-subdomain) is forced here for the same
    reason TODO 11's own backend block needs use_path_style = true -
    LocalStack doesn't answer to virtual-hosted-style bucket subdomains
    out of the box. Callers must check HAVE_BOTO3 before calling this."""
...
def _localstack_dynamodb_client():
    """A boto3 DynamoDB client pointed directly at LocalStack, same
    dummy credentials as every other boto3 client in this file. Callers
    must check HAVE_BOTO3 before calling this."""
    return boto3.client(
        "dynamodb",
        endpoint_url=LOCALSTACK_ENDPOINT_URL,
        region_name=AWS_REGION,
...
def ensure_backend_infra():
    """TODO 11 asks the student to configure a real S3 + DynamoDB
    Terraform backend - but Terraform can never create the very bucket
    and lock table its own backend block depends on: backend
    initialization happens before terraform touches a single resource in
    main.tf, so no resource block anywhere in this file could ever create
    them first. That's a genuine chicken-and-egg problem, not something
    fixable with cleverer HCL - so this grader creates both directly via
    boto3 instead, exactly once, unconditionally, before ANY check below
    runs (see main()) - not just check_todo_11().

    That "before any check, not just this TODO's own" part matters: once
    TODO 11 is solved, the backend block lives inside the ONE shared
    terraform {} block at the very top of main.tf, and every scoped copy
    this grader ever makes (see _scoped_workdir()) always includes that
    top block in full, no matter where main.tf gets truncated for some
    earlier TODO's own check. So the moment TODO 11 is solved, TODO 02's
    own check_todo_2() would try to `terraform init` against this same S3
    backend too - not because check_todo_2() changed, but because the
    backend it's now unavoidably wired to has to actually exist first.

    Idempotent - tolerates the bucket and table already existing, so
    repeated grading runs are safe. Returns True once both are confirmed
    to exist, False only if boto3 itself couldn't create or confirm one
    of them.

    Deliberately does NOT check existence first (e.g. head_bucket /
    describe_table) and only create on failure - some LocalStack versions
    answer HeadBucket with a false 200 for a bucket that was never
    actually created (a real, observed quirk, not a hypothetical one),
    which would make a check-then-create pattern silently skip creating
    the bucket. Instead this always attempts to create both, and simply
    tolerates the specific "already exists" error each service returns
    when they're already there - the standard idempotent-create idiom,
    and immune to that quirk either way."""
    if not HAVE_BOTO3:
        return False
    ...
    try:
        ...
        dynamodb = _localstack_dynamodb_client()
        try:
            dynamodb.create_table(
                TableName=BACKEND_DYNAMODB_TABLE,
        ...
        waiter.wait(TableName=BACKEND_DYNAMODB_TABLE)
        return True
    except Exception:
        return False
...
def check_todo_11():
    """Checks TODO 11 without ever reading main.tf's own source as text -
    consistent with every earlier check in this course. Instead, this
    reads back what Terraform itself resolved:

    1. terraform init in a fresh scoped copy - ensure_backend_infra() has
       already made sure the S3 bucket and DynamoDB table exist (see
       main()), so a correctly-written backend block succeeds here.

    2. Read .terraform/terraform.tfstate - a small pointer file
       `terraform init` itself writes the moment a real backend is
       configured (it is NOT the actual state; the actual state is what
       should be living in S3 instead). Confirm backend.type == "s3" and
       backend.config's bucket/key/dynamodb_table exactly match what this
       course requires.

    3. terraform apply for real. If the student typed the wrong
       dynamodb_table name, this is where it fails - LocalStack has no
       table by that name to lock against.

    4. Confirm NO local terraform.tfstate file exists in the scoped
       workdir afterward - once a real backend is configured, Terraform
       never falls back to writing state locally.

    5. Independently, via boto3 (never terraform show, never a Terraform
       output), confirm the state object genuinely exists in the S3
       bucket at the expected key.

    Always destroys before returning, pass or fail, so LocalStack is
    never left holding resources - same pattern as every apply-based
    check in this course. Runs against a copy truncated right before
    TODO 12's own work area (TODO_12_MARKER) - a broken, or not-yet-
    written, TODO 12 can't affect this result, consistent with every
    earlier check's "truncate before the next TODO's start" pattern."""
    if not HAVE_BOTO3:
        return False

    debug = os.environ.get("DEBUG_TODO11") == "1"

    def d(msg):
        if debug:
            console.print(c(f"  [DEBUG_TODO11] {msg}", DIM))

    workdir = _scoped_workdir(truncate_before=TODO_12_MARKER)
    try:
        rc, init_out, init_err = run_terraform(workdir, "init", "-input=false", "-no-color")
        if rc != 0:
            d(f"init failed (rc={rc}): {(init_err or init_out).strip()[:2000]}")
            return False
        d("init OK")

        pointer_file = workdir / ".terraform" / "terraform.tfstate"
        if not pointer_file.exists():
            d(f"{pointer_file} does not exist after init - no remote backend configured")
            return False
        try:
            pointer = json.loads(pointer_file.read_text(encoding="utf-8"))
        except json.JSONDecodeError:
            d("terraform.tfstate pointer file wasn't valid JSON")
            return False
        backend = pointer.get("backend") or {}
        d(f"backend pointer = {backend!r}")
        if backend.get("type") != "s3":
            d(f"backend.type was {backend.get('type')!r}, expected 's3'")
            return False
        config = backend.get("config") or {}
        if config.get("bucket") != BACKEND_BUCKET:
            d(f"backend bucket was {config.get('bucket')!r}, expected {BACKEND_BUCKET!r}")
            return False
        if config.get("key") != BACKEND_KEY:
            d(f"backend key was {config.get('key')!r}, expected {BACKEND_KEY!r}")
            return False
        if config.get("dynamodb_table") != BACKEND_DYNAMODB_TABLE:
            d(f"backend dynamodb_table was {config.get('dynamodb_table')!r}, expected {BACKEND_DYNAMODB_TABLE!r}")
            return False
        d("backend pointer file matches required bucket/key/dynamodb_table")

        rc, _, err = run_terraform(workdir, "apply", "-auto-approve", "-input=false", "-no-color")
        if rc != 0:
            d(f"apply failed (rc={rc}): {err.strip()[:2000]}")
            return False
        d("apply OK")

        local_state_file = workdir / "terraform.tfstate"
        if local_state_file.exists():
            d(f"a LOCAL terraform.tfstate was created at {local_state_file} - state is not really remote")
            return False
        d("no local terraform.tfstate file - state is not sitting on local disk")

        try:
            s3 = _localstack_s3_client()
            head = s3.head_object(Bucket=BACKEND_BUCKET, Key=BACKEND_KEY)
        except Exception as exc:
            d(f"boto3 head_object raised: {exc!r}")
            return False
        if not head.get("ContentLength"):
            d(f"S3 object at {BACKEND_KEY} was empty or missing: {head!r}")
            return False
        d(f"S3 object confirmed independently, ContentLength={head.get('ContentLength')}")
...
def _new_audit_timestamp():
    """A fresh, unique-enough RFC 3339 UTC timestamp string, in the exact
    shape Terraform's own timestamp() function produces (the shape every
    entry already in audit/apply_log.json uses) - used to simulate "time
    has passed and this pipeline got applied again" between check_todo_12()'s
    two applies, entirely inside that check's own scratch copy of the
    file. Second-resolution is more than enough: this is compared only
    against the single fixed seed timestamp shipped in this folder's own
    audit/apply_log.json, never against another grading run's value."""
    return datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")


def check_todo_12():
    """Checks TODO 12 by proving the audit trail is genuinely additive,
    not just present. All within ONE scoped workdir (never touching this
    folder's real audit/apply_log.json):

    1. terraform init + apply for real. Read back audit/apply_log.json AS
       COPIED into this workdir (its one seed timestamp), and confirm
       output "apply_audit_log_keys" has exactly one "audit-log/<ts>.json"
       key per timestamp in that file - never by reading main.tf's own
       source as text.

    2. Independently, via boto3 (never terraform show, never a Terraform
       output), confirm every one of those keys genuinely exists as an S3
       object, with the correct site_count, gateway_count, and sites in
       its JSON content.

    3. Append ONE new, never-seen-before timestamp directly to THIS
       workdir's own audit/apply_log.json - simulating time passing and
       this pipeline being applied again, exactly the way check_todo_3
       and check_todo_7 temporarily edit a fixture file to simulate a
       scenario, except this edit is never reverted (an audit trail is
       supposed to grow).

    4. Plan again against that SAME state (no destroy in between) and read
       the plan's own resource_changes: every instance keyed by an
       ALREADY-existing timestamp must plan as "no-op" - proof
       aws_s3_object.apply_audit_log's own key never changed for them, so
       nothing about them is being touched, let alone replaced - while the
       instance keyed by the brand-new timestamp must plan as "create".

    5. Apply that plan for real, confirm output "apply_audit_log_keys" now
       has one MORE key than before, and independently re-confirm via
       boto3 that every key - old and new alike - still exists in S3.

    Always destroys before returning, pass or fail, so LocalStack is never
    left holding resources - same pattern as every apply-based check in
    this course. Runs against the full, untruncated file - TODO 12 is the
    newest TODO in this folder, so nothing trails it that would need
    isolating."""
    if not HAVE_BOTO3:
        return False

    debug = os.environ.get("DEBUG_TODO12") == "1"

    def d(msg):
        if debug:
            console.print(c(f"  [DEBUG_TODO12] {msg}", DIM))

    site_ids = ("rdu01", "aus02", "sea03")
    expected_devices = _compute_expected_branch_gateway_devices()
    if expected_devices is None:
        d("_compute_expected_branch_gateway_devices() returned None")
        return False
    expected_gateway_count = len(expected_devices)
    expected_sites = sorted(site_ids)

    def _expected_keys(entries):
        return sorted(f"{AUDIT_KEY_PREFIX}{ts}.json" for ts in entries)

    def _verify_object(s3, key):
        try:
            obj = s3.get_object(Bucket=BACKEND_BUCKET, Key=key)
            body = json.loads(obj["Body"].read())
        except Exception as exc:
            d(f"boto3 get_object({key!r}) raised or wasn't valid JSON: {exc!r}")
            return False
        if not body.get("applied_at"):
            d(f"{key} has no applied_at: {body!r}")
            return False
        if body.get("site_count") != len(site_ids):
            d(f"{key} site_count was {body.get('site_count')!r}, expected {len(site_ids)}")
            return False
        if body.get("gateway_count") != expected_gateway_count:
            d(f"{key} gateway_count was {body.get('gateway_count')!r}, expected {expected_gateway_count}")
            return False
        if sorted(body.get("sites") or []) != expected_sites:
            d(f"{key} sites was {body.get('sites')!r}, expected {expected_sites}")
            return False
        return True
    ...
    try:
        ...
        audit_file = workdir / AUDIT_LOG_FILE.relative_to(ROOT)
        try:
            entries_before = json.loads(audit_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError) as exc:
            d(f"couldn't read {audit_file}: {exc!r}")
            return False
        if not isinstance(entries_before, list) or not entries_before:
            d(f"audit/apply_log.json wasn't a non-empty list: {entries_before!r}")
            return False
        d(f"entries_before = {entries_before!r}")
        ...
        rc, stdout, err = run_terraform(workdir, "output", "-json", "apply_audit_log_keys")
        if rc != 0:
            d(f"reading apply_audit_log_keys failed (rc={rc}): {err.strip()[:2000]}")
            return False
        try:
            keys_first = json.loads(stdout)
        except json.JSONDecodeError:
            d(f"apply_audit_log_keys wasn't valid JSON: {stdout[:2000]!r}")
            return False
        expected_keys_first = _expected_keys(entries_before)
        if sorted(keys_first or []) != expected_keys_first:
            d(f"apply_audit_log_keys was {keys_first!r}, expected {expected_keys_first!r}")
            return False
        d(f"apply_audit_log_keys after first apply = {keys_first!r}")

        s3 = _localstack_s3_client()
        for key in expected_keys_first:
            if not _verify_object(s3, key):
                return False
        d("every pre-existing audit object independently confirmed in S3")

        new_timestamp = _new_audit_timestamp()
        while new_timestamp in entries_before:
            new_timestamp = _new_audit_timestamp()
        entries_after = entries_before + [new_timestamp]
        audit_file.write_text(json.dumps(entries_after, indent=2) + "\n", encoding="utf-8")
        d(f"appended new timestamp {new_timestamp!r} to this workdir's own audit/apply_log.json")

        plan_file = workdir / "tfplan_audit.out"
        rc, _, err = run_terraform(workdir, "plan", f"-out={plan_file}", "-input=false", "-no-color")
        if rc != 0:
            d(f"second plan failed (rc={rc}): {err.strip()[:2000]}")
            return False
        rc, stdout, err = run_terraform(workdir, "show", "-json", str(plan_file))
        if rc != 0:
            d(f"terraform show -json on second plan failed (rc={rc}): {err.strip()[:2000]}")
            return False
        try:
            plan = json.loads(stdout)
        except json.JSONDecodeError:
            d("second plan's show -json output wasn't valid JSON")
            return False

        actions = {}
        for change in plan.get("resource_changes", []):
            address = change.get("address", "")
            if address.startswith("aws_s3_object.apply_audit_log["):
                actions[address] = change.get("change", {}).get("actions", [])
        d(f"aws_s3_object.apply_audit_log planned actions: {actions!r}")

        for ts in entries_before:
            address = f'aws_s3_object.apply_audit_log["{ts}"]'
            if actions.get(address, ["no-op"]) != ["no-op"]:
                d(f"{address} was NOT no-op on the second plan - an existing entry would be touched/replaced")
                return False
        new_address = f'aws_s3_object.apply_audit_log["{new_timestamp}"]'
        if actions.get(new_address) != ["create"]:
            d(f"{new_address} was {actions.get(new_address)!r}, expected ['create']")
            return False
        d("existing entries all no-op, exactly the new timestamp plans to create - as expected")

        rc, _, err = run_terraform(workdir, "apply", "-auto-approve", "-input=false", "-no-color")
        if rc != 0:
            d(f"second apply failed (rc={rc}): {err.strip()[:2000]}")
            return False
        d("second apply OK")

        rc, stdout, err = run_terraform(workdir, "output", "-json", "apply_audit_log_keys")
        if rc != 0:
            d(f"reading apply_audit_log_keys (2nd time) failed (rc={rc}): {err.strip()[:2000]}")
            return False
        try:
            keys_second = json.loads(stdout)
        except json.JSONDecodeError:
            d(f"apply_audit_log_keys wasn't valid JSON: {stdout[:2000]!r}")
            return False
        expected_keys_second = _expected_keys(entries_after)
        if sorted(keys_second or []) != expected_keys_second:
            d(f"apply_audit_log_keys was {keys_second!r}, expected {expected_keys_second!r}")
            return False
        d(f"apply_audit_log_keys after second apply = {keys_second!r}")

        for key in expected_keys_second:
            if not _verify_object(s3, key):
                return False
        d("every audit object (old and new) independently confirmed in S3 after the second apply")
...
def compute_statuses():
    ...
    statuses[11] = check_todo_11() if all(statuses[n] for n in range(1, 11)) else False
    statuses[12] = check_todo_12() if all(statuses[n] for n in range(1, 12)) else False
    return statuses
...
def print_todo_details(number, statuses):
    if number == 1:
        ...
        console.print("[11] Configuring the remote state backend and locking...")
        if statuses[number]:
            console.print(f"  terraform init -> backend \"s3\" resolved (bucket={BACKEND_BUCKET})")
            console.print(f"    key            -> {BACKEND_KEY}")
            console.print(f"    dynamodb_table -> {BACKEND_DYNAMODB_TABLE}")
            console.print("  terraform apply -> succeeded against the real S3 + DynamoDB backend")
            console.print("  local terraform.tfstate -> absent, state is not sitting on local disk")
            console.print(f"  s3://{BACKEND_BUCKET}/{BACKEND_KEY} -> confirmed independently via the AWS API")
    elif number == 12:
        console.print("[12] Emitting a structured audit log...")
        if statuses[number]:
            console.print("  terraform apply -> 1 audit record written for the existing timestamp")
            console.print("  audit/apply_log.json -> 1 new timestamp appended, simulating time passing")
            console.print("  terraform plan  -> every earlier audit record -> no-op")
            console.print("  terraform plan  -> the new timestamp's own record -> create")
            console.print("  terraform apply -> reconciled, no earlier record touched or replaced")
            console.print(f"  s3://{BACKEND_BUCKET}/{AUDIT_KEY_PREFIX}*.json -> every record confirmed independently via the AWS API")
...
def print_localstack_unreachable():
    ...
    console.print(c(
        f"Couldn't reach {LOCALSTACK_HEALTH_URL} - TODO 06 through TODO 12 all need "
...
def print_boto3_missing():
    ...
    console.print(c(
        "TODO 10, TODO 11, and TODO 12 all use boto3 to call the AWS API "
        "directly, independently of Terraform. Install it, then run python "
        "grading.py again.",
        YELLOW,
    ))
    console.print()
...
def print_backend_bootstrap_failed():
    banner("Could not prepare the S3 backend and DynamoDB lock table", YELLOW)
    console.print(c(
        "TODO 11's backend block depends on an S3 bucket and a DynamoDB "
        "table that Terraform can never create itself (a backend has to "
        "already exist before terraform init can use it). This grader "
        "tries to create both directly via boto3 before any check runs, "
        f"and that attempt failed - confirm LocalStack at {LOCALSTACK_ENDPOINT_URL} "
        "is fully up, then run python grading.py again.",
        YELLOW,
    ))
    console.print()
...
def main():
    ...
    if shutil.which("terraform") is None:
        ...
        console.print(c("Install Terraform >= 1.6 (https://developer.hashicorp.com/terraform/install) and try again.", YELLOW))
        console.print()
        sys.exit(127)
    ...
    if not ensure_backend_infra():
        print_backend_bootstrap_failed()
        sys.exit(1)
