...
TODO 01, and stops at the first incomplete one - even though TODO 01-06
...
TODO 07 doesn't create anything new; it adds check blocks that verify
what TODO 06 created actually matches a fixed, expected shape. Every
check below still runs against a disposable scratch copy of this lab
(see _scoped_workdir()), exactly like TODO 02-06 - the student's real
terraform.tfstate in this directory is never touched by grading. Every
check that applies the full file (TODO 06's and TODO 07's own) runs
`terraform destroy` in that same scratch copy before finishing - pass or
fail - so LocalStack is never left holding orphaned VPCs/subnets/
instances between grading runs (see apply_read_and_destroy()).
...
TODO_07_MARKER = "# STUDENT WORK AREA - TODO 07"

# The check blocks after TODO 07's own work area reference aws_subnet.zone
# and aws_instance.branch_gateway - resources that already exist (TODO 06
# is solved), so referencing them is never the problem. But a check block
# with a syntax mistake, or none written at all, is a completely normal,
# expected state before TODO 07 is solved - not a syntax error - so
...
CURRENT_TODO_END_MARKER = "# END STUDENT WORK AREA - TODO 07"
...
def check_todo_7():
    ...
    always say "pass". Applies the full (untruncated) file twice against
    LocalStack, in a fresh scratch copy each time (same real-apply
    approach as check_todo_6): once against a version of
    sites/rdu01.json with one of its 2 firewalls entries temporarily
    removed (rdu01 should then only get 1 branch gateway instance
    instead of 2), confirming "gateway_counts_match_expected" reports
    status "fail"; once against the real, unmodified file, confirming
    both checks report status "pass". Always restores sites/rdu01.json
    in a finally block, even if something raises.
    ...
    try:
        ...
        broken_results = apply_read_and_destroy(names)
    except (json.JSONDecodeError, UnicodeDecodeError):
        return False
    finally:
    ...
    clean_results = apply_read_and_destroy(names)
...
def print_localstack_unreachable():
    ...
    console.print(c(
        f"Couldn't reach {LOCALSTACK_HEALTH_URL} - TODO 06 and TODO 07 both need "
