...
TODO 01, and stops at the first incomplete one - even though TODO 01-05
...
local.branch_gateway_devices - pure data, no resources. TODO 06 is the
first TODO in this course to create real resources - VPCs, subnets, a
route table, a security group, and an EC2 instance, per site - against
LocalStack (a local AWS emulator), using the real, official hashicorp/aws
provider. Every check below still runs against a disposable scratch copy
of this lab (see _scoped_workdir()), exactly like TODO 02-05 - the
student's real terraform.tfstate in this directory is never touched by
grading. TODO 06's check goes one step further than any earlier TODO's,
though: because it's the first check that creates REAL resources in
LocalStack, it runs `terraform destroy` in that same scratch copy before
finishing - pass or fail - so LocalStack is never left holding orphaned
VPCs/subnets/instances between grading runs (see apply_read_and_destroy()).
...
TODO_06_MARKER = "# STUDENT WORK AREA - TODO 06"

# The 6 "provided" output blocks after TODO 06's own work area reference
# aws_vpc.site, aws_subnet.zone, etc. - resources that don't exist until
# TODO 06 is actually solved. Before that, referencing them is a
# completely normal, expected state - not a syntax error - so
# ensure_valid() validates only up through here, never those trailing
# outputs. See ensure_valid() for why this matters (same fix as TODO 04's
# grading.py, now applied proactively here for the same reason).
CURRENT_TODO_END_MARKER = "# END STUDENT WORK AREA - TODO 06"
...
def apply_read_and_destroy(names):
    """Like apply_and_read_output, but for TODO 06, where a full
    (untruncated) apply creates REAL mock resources in LocalStack -
    something no earlier TODO in this course ever did. Every apply here
    is followed by `terraform destroy`, before the scratch copy itself
    is deleted, so LocalStack never accumulates orphaned VPCs, subnets,
    or instances across repeated grading runs - this check leaves
    LocalStack exactly as it found it, pass or fail.

    Returns a dict {name: parsed_output_or_None} for every name in
    `names`, plus one extra key "_apply_ok" (bool) so callers can tell
    "apply failed" apart from "apply succeeded but this output happened
    to be null"."""
...
def check_todo_6():
    """Runs against the full, untruncated file - TODO 06 is the last
    TODO in this folder, so there's nothing later to cut away. This is
    the first check in this course that creates real resources in
    LocalStack (see apply_read_and_destroy()), and the first that
    requires LocalStack to be reachable at all (see main())."""
    names = ["vpcs", "subnets", "route_tables", "route_table_associations", "security_groups", "branch_gateways"]
    results = apply_read_and_destroy(names)
...
def print_localstack_unreachable():
    ...
    console.print(c(
        f"Couldn't reach {LOCALSTACK_HEALTH_URL} - TODO 06 needs LocalStack running "
        "before it can create anything. See TASK.md's \"Before You Start\" section.",
        YELLOW,
    ))
    console.print()
